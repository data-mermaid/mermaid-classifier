...
The full design lives at `/Users/gregn/.claude/plans/i-have-just-finished-jiggly-duckling.md`. Implementation: `scripts/classifier_train_balancing_sweep.py`. New strategy: `'soft_balanced'` allocator in `mermaid_classifier/training/subsample/registry.py` with formula `target_c ∝ n_c^(1-alpha)`.
...
- **The new `soft_balanced` strategy is in production code.** It's tested (`tests/training/test_subsample/`), deterministic under DuckDB parallel scans, and registered in `_ALLOCATORS`. No follow-up work is required there unless a use case for `alpha < 0` (oversampling) or `alpha > 1` (over-balanced) emerges.
- **Infrastructure note**: this run hit a real MLflow concurrency bug — when two `ProcessPoolExecutor` workers both call `mlflow.set_experiment` on a not-yet-existing experiment, one wins the implicit `create_experiment` and the other silently falls back to `Default`. The sweep script now pre-creates both experiments in the parent process via `_ensure_experiments_exist()` before any worker spawns. Any future multi-process MLflow training script in this repo should do the same.
